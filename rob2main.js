    ...
    $('.gen-button').click(function(){
        if($('.gen-area').hasClass('area-disabled') || $account_connected == false) {
            ...
            pokeCoins = $('.gen-section .gen-area .rp-list li.activated').text();
              $('.pending-pokecoins').html('Warten.. <img class="pokecoins-icon" src="https://i.imgur.com/W6W128K.png"> ' + pokeCoins + 'Robux für <span class="username-modal">' + $('#usernameInput').val() + '</span>');
            $('.pokecoins-amt').html('<img class="pokecoins-icon" src="https://i.imgur.com/W6W128K.png"> ' + pokeCoins);
           if($selected_rp != ''){
                $.magnificPopup.open({
                    items: {
                        src: '#gen_modal',
                ...
                sweetAlert("Error", "Bitte wählen Sie die Anzahl der Robux, die Sie generieren möchten.", "error");
            }
        }
    });
            ...
            progress_fast(function(){
                ...
                progress_fast(function(){
                    $message_span.text('Serverinformationen abrufen');
                        progress_fast(function(){
                            $message_span.html('Generieren <img class="pokecoins-icon" src="https://i.imgur.com/W6W128K.png"> ' + pokeCoins + 'Robux..');
                            progress_fast(function(){
